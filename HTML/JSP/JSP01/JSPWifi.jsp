...
File f = new File("/var/lib/tomcat9/webapps/ROOT/전국무료와이파이표준데이터Refine.csv");
...
int LineCnt = 0; 
readtxt = filefield.get(LineCnt);
for(int i = from -1 ; i < from+cnt-1; i++){
    if(i + 1 > total_cnt) break; // 총 파일의 갯수를 넘어가면 break;
...
<tr style = "text-align : center">
    ...
    <td><%=field[14]%></td>
    <td><%=dist%></td>
</tr> <!--field로 나눈 배열을 이용해서 번호 주소 위도 경도 거리를 출력-->
...
<div style = 'text-align : center'> <!--밑에 페이지를 출력하기 위한 객체-->
<form method ="get" action ="JSPWifi.jsp"> <!--페이지를 클릭했을때 action과 get함수를 이용해서 parameter 전달-->
<%
if(start_pagenumber==0){
} else{
%>
    <a href="JSPWifi.jsp?from=<%=((start_pagenumber - 10) * cnt + 1)%>&cnt=<%=cnt%>">&lt;&lt;</a>
<%
