...
public class example13a {
	...
	public static Calendar k05_cal = Calendar.getInstance();  //cal 현재날짜받아오기
	public static SimpleDateFormat k05_dt = new SimpleDateFormat("YYYY.MM.dd HH:mm:ss"); // 현재시간 data형식
	...
	public static void k05_ItemPrint(int k05_p) {
		...
		int k05_page = 1;
			...
			System.out.printf("%04d  %5.5s         %4.4s      %4.4s      %4.4s      %4.4s    %6.6s\n",
					...
					k05_rec.k05_sum(), k05_rec.k05_avg()); //번호에맞는 이름 국어 영어 수학점수 총점 평균 출력
			...
			if(((k05_p + 1) % 30) == 0 || k05_p == (k05_iPerson - 1)) {  // 30개가 지났을때와 마지막에는 30개가 안되도 출력
				...
				k05_mathavg = k05_mathsum / (k05_p + 1); // 30개씩의 평균과 누적평균계산
				System.out.printf("==================================================================\n");
